...
from Three_DGame.util.db import REDISCLIENT


class ThreeDgameSpiderMiddleware(object):
...
class ThreeDgameDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
...
class ProxyMiddleware(object):
    ...
    def process_response(self, request, response, spider):
        if "频繁" in response.text:
            spider.logger.info("当前的代理失效")
            url = response.url
            request.meta['proxy'] = None
            spider.logger.info("将当前请求[%s]重新加入请求队列" % url)
